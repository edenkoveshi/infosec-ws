...
decision_t* inspect_pkt(struct sk_buff *skb,direction_t dir){
	...
	if(iph->protocol == PROT_TCP){
		...
		if(tcph->rst){
			...
			remove_conn_from_table(conn,compare_conn);
			remove_conn_from_table(rev_conn,compare_conn);
			...
			if(tcph->ack){ //syn + ack
				...
				if(lookup(conn,compare_conn) != NULL){
					kfree(conn);
					res->action = NF_DROP;
					res->reason = rule_num;
					return res;
				...
				if(add_connection(conn) == ERROR){
					printk(KERN_INFO "failed at add_connection 2\n");
					kfree(conn);
					res->action = NF_DROP;
					res->reason = REASON_ILLEGAL_VALUE;
					return res;	

...
//static unsigned int last_read_log;
static DEFINE_SPINLOCK(xxx_lock);

/*EXPORT_SYMBOL(sysfs_class); //https://stackoverflow.com/questions/25446203/how-to-export-a-struct-between-two-kernel-modules-using-export-symbol-or-equival
EXPORT_SYMBOL(new_log);*/
...
int compare_logs(log_row_t* a,log_row_t* b){
	...
	//printk(KERN_INFO "ip's are the same");
...
int show_logs(struct file* file,char __user* buffer,size_t length,loff_t* offset){
	...
	/*while(temp < last_read_log){
		...
		if(node == NULL){
			...
			return 0; //NOTE - this returns 0 which means all logs have been read
		}
		temp++;
	}*/
	...
	while(node!=NULL){
		...
		if(node->log == NULL){
			...
			return ERROR; //NOTE - this returns 0 which means all logs have been read
		}
		...
		/*printk(KERN_INFO "%s %s %s %u %u %s %u %s %d %u\n",timestamp,src_ip,dst_ip,log->src_port,log->dst_port,
			protocol,log->hooknum,action,log->reason,log->count);*/
